...
use screeps_combat_agent::opponents::{
    run_engagement, world_from_units, DrainAgent, RushAgent, TurtleAgent, Unit,
};
use screeps_combat_agent::IbexAgent;
use screeps_combat_engine::{CombatWorld, MovementState, SimBody, SimCreep, SimTower};
...
fn main() {
    ...
    // 3) A ranged QUAD vs a beefy (10-HEAL) turtle — a different composition.
    {
        let world = world_from_units(
            ...
            &[Unit::new(
                vec![(Part::RangedAttack, 7)],
                vec![p(24, 22), p(25, 22), p(26, 22), p(25, 21)],
            )],
            1,
    ...
    // 4) A drain tank vs a tower: self-heals through falloff fire, bleeds the tower's energy.
    {
        ...
        let out = run_engagement(
            ...
            1,
            p(25, 22),
            &mut screeps_combat_agent::HoldAgent,
            15,
        );
...
fn write(
    dir: &std::path::Path,
    name: &str,
    out: &screeps_combat_agent::opponents::EngagementOutcome,
) {
